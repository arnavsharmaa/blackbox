...
Pure data + conversion helpers with no ROS dependency, so the mapping can be
unit-tested anywhere and reused by both a rosbag2 batch adapter and the live
recorder node in robotics/ros2/. Message payloads are plain dicts shaped like
their ROS counterparts (the recorder node and a bag reader both produce
those).
"""
