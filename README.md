...
| `GET /api/incidents` | Paginated list; filters: `robot_id`, `facility`, `severity`, `outcome`, `failure_category`, `start_after`, `start_before`, `q` (free-text over id/task/summary/robot), `limit`, `offset` |
| `GET /api/incidents/export.csv` | The filtered list as CSV, for spreadsheets and notebooks (same filters as the list) |
